...
use super::{commit_history, time_distribution};
...
pub(crate) const GRAPHQL_ENDPOINT: &str = "https://api.github.com/graphql";
...
impl GraphQLClient {
    ...
    pub async fn fetch_stats(&self, username: &str) -> Result<UserStats> {
        ...
        eprintln!(
            "fetching commit history (repos ≤ {}, ≤ {} commits/repo, ≤ {} total)...",
            self.language_config.repos_limit,
            self.language_config.commits_per_repo,
            self.language_config.commits_limit
        );
        let contributed = contributed_repos(&user.contributions_collection);
        let histories = commit_history::fetch_commit_history(
            &self.client,
            &contributed,
            &user.id,
            &one_year_ago,
        ...
        let commit_dates: Vec<DateTime<Utc>> = histories
            .iter()
            .flat_map(|h| h.commits.iter().map(|c| c.authored_date))
            .collect();

        eprintln!("computing time distribution...");
        let time_distribution = Some(time_distribution::compute_time_distribution(
            &commit_dates,
            self.timezone_offset,
        ));

        let commit_count = commit_dates.len();
        eprintln!("computing language usage ({commit_count} commits)...");
        let language_stats = {
            let (usage, total_changes, sampled) =
                commit_history::compute_language_usage(&histories, &self.language_config);
            if total_changes > 0 {
                Some((usage, total_changes, sampled))
            } else {
                None
            }
        };
...
/// Extract (owner, name) pairs of repositories the user committed to,
/// sorted by contribution count descending.
fn contributed_repos(collection: &ContributionsCollectionNode) -> Vec<(String, String)> {
    let mut repos: Vec<(String, String, u64)> = collection
        .commit_contributions_by_repository
        .iter()
        .filter_map(|c| {
            let (owner, name) = c.repository.name_with_owner.split_once('/')?;
            Some((
                owner.to_string(),
                name.to_string(),
        ...
        .collect();
    repos.sort_by_key(|r| std::cmp::Reverse(r.2));
    repos
        .into_iter()
